...
import type { CodianMcpServer, McpServerConfig, McpServerType } from '../../../core/types';
...
export class McpSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'codian-mcp-header' });
    headerEl.createSpan({ text: 'MCP Servers', cls: 'codian-mcp-label' });

    const addContainer = headerEl.createDiv({ cls: 'codian-mcp-add-container' });
    ...
    const httpOption = dropdown.createDiv({ cls: 'codian-mcp-add-option' });
    setIcon(httpOption.createSpan({ cls: 'codian-mcp-add-option-icon' }), 'globe');
    ...
    const importOption = dropdown.createDiv({ cls: 'codian-mcp-add-option' });
    setIcon(importOption.createSpan({ cls: 'codian-mcp-add-option-icon' }), 'clipboard-paste');
    ...
    if (this.servers.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'codian-mcp-empty' });
      emptyEl.setText('No MCP servers configured. Click "Add" to add one.');
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'codian-mcp-list' });
  ...
  private renderServerItem(listEl: HTMLElement, server: CodianMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'codian-mcp-item' });
    if (!server.enabled) {
      itemEl.addClass('codian-mcp-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'codian-mcp-status' });
    statusEl.addClass(
      server.enabled ? 'codian-mcp-status-enabled' : 'codian-mcp-status-disabled'
    );

    const infoEl = itemEl.createDiv({ cls: 'codian-mcp-info' });

    const nameRow = infoEl.createDiv({ cls: 'codian-mcp-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'codian-mcp-name' });
    nameEl.setText(server.name);

    const serverType = getMcpServerType(server.config);
    const typeEl = nameRow.createSpan({ cls: 'codian-mcp-type-badge' });
    typeEl.setText(serverType);

    if (server.contextSaving) {
      const csEl = nameRow.createSpan({ cls: 'codian-mcp-context-saving-badge' });
      csEl.setText('@');
      csEl.setAttribute('title', 'Context-saving: mention with @' + server.name + ' to enable');
    }

    const previewEl = infoEl.createDiv({ cls: 'codian-mcp-preview' });
    ...
    const actionsEl = itemEl.createDiv({ cls: 'codian-mcp-actions' });

    const testBtn = actionsEl.createEl('button', {
      cls: 'codian-mcp-action-btn',
    ...
    const deleteBtn = actionsEl.createEl('button', {
      cls: 'codian-mcp-action-btn codian-mcp-delete-btn',
  ...
  private async testServer(server: CodianMcpServer) {
  ...
  private async updateAllDisabledTools(server: CodianMcpServer, disabledTools: string[]) {
    await this.updateServerDisabledTools(
      server,
  ...
  private getServerPreview(server: CodianMcpServer, type: McpServerType): string {
    if (type === 'stdio') {
  ...
  private openModal(existing: CodianMcpServer | null, initialType?: McpServerType) {
  ...
  private async saveServer(server: CodianMcpServer, existing: CodianMcpServer | null) {
    if (existing) {
  ...
  private async toggleServer(server: CodianMcpServer) {
  ...
  private async deleteServer(server: CodianMcpServer) {
