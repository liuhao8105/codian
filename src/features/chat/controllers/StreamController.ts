...
export class StreamController {
  ...
  private handleRegularToolUse(
    ...
    if (existingToolCall) {
      ...
      if (Object.keys(newInput).length > 0) {
        ...
        if (toolEl) {
          const nameEl = toolEl.querySelector('.codian-tool-name') as HTMLElement | null
            ?? toolEl.querySelector('.codian-write-edit-name') as HTMLElement | null;
          if (nameEl) {
            nameEl.setText(getToolName(existingToolCall.name, existingToolCall.input));
          }
          const summaryEl = toolEl.querySelector('.codian-tool-summary') as HTMLElement | null
            ?? toolEl.querySelector('.codian-write-edit-summary') as HTMLElement | null;
          if (summaryEl) {
  ...
  async appendText(text: string): Promise<void> {
    ...
    if (!state.currentTextEl) {
      state.currentTextEl = state.currentContentEl.createDiv({ cls: 'codian-text-block' });
  ...
  showThinkingIndicator(overrideText?: string, overrideCls?: string): void {
    ...
    state.thinkingIndicatorTimeout = setTimeout(() => {
      ...
      const timerSpan = state.thinkingEl.createSpan({ cls: 'codian-thinking-hint' });
      const updateTimer = () => {
  ...
  private renderCompactBoundary(): void {
    ...
    if (!state.currentContentEl) return;
    this.hideThinkingIndicator();
    const el = state.currentContentEl.createDiv({ cls: 'codian-compact-boundary' });
    el.createSpan({ cls: 'codian-compact-boundary-label', text: 'Conversation compacted' });
  }
