...
function mergeImages(existing: ChatMessage['images'], incoming: ChatMessage['images']): ChatMessage['images'] {
  ...
  const seen = new Set(merged.map((image) => `${image.name}:${image.size}`));
...
function appendEmbeddedImageOrder(prompt: string, references: Array<{ index: number; fileName: string; embedPath: string }>): string {
  if (references.length === 0) {
    return prompt;
  }

  const lines = references.map((ref) => `- Image ${ref.index}: ${ref.fileName} (embed: ![[${ref.embedPath}]])`);
...
export interface InputControllerDeps {
  ...
  getInstructionRefineService: () => InstructionRuntime | null;
