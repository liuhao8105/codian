...
export async function initializeTabService(
  ...
  let service: AgentRuntime | null = null;
  let unsubscribeReadyState: (() => void) | null = null;

  try {
    // Create per-tab agent runtime
    service = createAgentRuntime(plugin, mcpManager);
 ...
 */
function initializeContextManagers(tab: TabData, plugin: CodianPlugin): void {
  const { dom } = tab;
 ...
 */
function initializeInstructionAndTodo(tab: TabData, plugin: CodianPlugin): void {
  const { dom } = tab;

  tab.services.instructionRefineService = createInstructionRuntime(plugin);
  tab.services.titleGenerationService = createTitleRuntime(plugin);
 ...
 */
function resolveForkSource(tab: TabData, plugin: CodianPlugin): ForkSource | null {
 ...
 */
export function wireTabInputEvents(tab: TabData, plugin: CodianPlugin): void {
 ...
 */
export function getTabTitle(tab: TabData, plugin: CodianPlugin): string {
  if (tab.conversationId) {
    const conversation = plugin.getConversationSync(tab.conversationId);
...
export function setupServiceCallbacks(tab: TabData, plugin: CodianPlugin): void {
