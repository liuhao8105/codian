...
export class InlineAskUserQuestion {
  ...
  constructor(
    ...
    this.signal = signal;
    this.config = {
      title: config?.title ?? 'Codian has a question',
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'codian-ask-question-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'codian-ask-inline-title' });
    ...
    if (!this.config.immediateSelect) {
      this.tabBar = this.rootEl.createDiv({ cls: 'codian-ask-tab-bar' });
      this.renderTabBar();
    }
    this.contentArea = this.rootEl.createDiv({ cls: 'codian-ask-content' });
  ...
  private renderTabBar(): void {
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tab = this.tabBar.createSpan({ cls: 'codian-ask-tab' });
      tab.createSpan({ text: this.questions[idx].header, cls: 'codian-ask-tab-label' });
      tab.createSpan({ text: answered ? ' \u2713' : '', cls: 'codian-ask-tab-tick' });
    ...
    const submitTab = this.tabBar.createSpan({ cls: 'codian-ask-tab' });
    submitTab.createSpan({ text: allAnswered ? '\u2713 ' : '', cls: 'codian-ask-tab-submit-check' });
    submitTab.createSpan({ text: 'Submit', cls: 'codian-ask-tab-label' });
  ...
  private renderQuestionTab(idx: number): void {
    ...
    this.contentArea.createDiv({
      text: q.question,
      cls: 'codian-ask-question-text',
    });

    const listEl = this.contentArea.createDiv({ cls: 'codian-ask-list' });
    ...
    for (let optIdx = 0; optIdx < q.options.length; optIdx++) {
      ...
      const row = listEl.createDiv({ cls: 'codian-ask-item' });
      if (isFocused) row.addClass('is-focused');
      if (isSelected) row.addClass('is-selected');

      row.createSpan({ text: isFocused ? '\u203A' : '\u00A0', cls: 'codian-ask-cursor' });
      row.createSpan({ text: `${optIdx + 1}. `, cls: 'codian-ask-item-num' });

      if (isMulti) {
        this.renderMultiSelectCheckbox(row, isSelected);
      }

      const labelBlock = row.createDiv({ cls: 'codian-ask-item-content' });
      const labelRow = labelBlock.createDiv({ cls: 'codian-ask-label-row' });
      labelRow.createSpan({ text: option.label, cls: 'codian-ask-item-label' });

      if (!isMulti && isSelected) {
        labelRow.createSpan({ text: ' \u2713', cls: 'codian-ask-check-mark' });
      }

      if (option.description) {
        labelBlock.createDiv({ text: option.description, cls: 'codian-ask-item-desc' });
      }
    ...
    if (this.config.showCustomInput) {
      ...
      const customRow = listEl.createDiv({ cls: 'codian-ask-item codian-ask-custom-item' });
      if (customFocused) customRow.addClass('is-focused');

      customRow.createSpan({ text: customFocused ? '\u203A' : '\u00A0', cls: 'codian-ask-cursor' });
      customRow.createSpan({ text: `${customIdx + 1}. `, cls: 'codian-ask-item-num' });
  ...
  private renderSubmitTab(): void {
    this.contentArea.createDiv({
      ...
      cls: 'codian-ask-review-title',
    });

    const reviewEl = this.contentArea.createDiv({ cls: 'codian-ask-review' });
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      pairEl.createDiv({ text: `${idx + 1}.`, cls: 'codian-ask-review-num' });
      ...
      bodyEl.createDiv({
        ...
        cls: answerText ? 'codian-ask-review-a-text' : 'codian-ask-review-empty',
      });
    ...
    this.contentArea.createDiv({
      ...
      cls: 'codian-ask-review-prompt',
    });

    const actionsEl = this.contentArea.createDiv({ cls: 'codian-ask-list' });
    const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));

    const submitRow = actionsEl.createDiv({ cls: 'codian-ask-item' });
    if (this.focusedItemIndex === 0) submitRow.addClass('is-focused');
    if (!allAnswered) submitRow.addClass('is-disabled');
    submitRow.createSpan({ text: this.focusedItemIndex === 0 ? '\u203A' : '\u00A0', cls: 'codian-ask-cursor' });
    submitRow.createSpan({ text: '1. ', cls: 'codian-ask-item-num' });
    submitRow.createSpan({ text: 'Submit answers', cls: 'codian-ask-item-label' });
    ...
    const cancelRow = actionsEl.createDiv({ cls: 'codian-ask-item' });
    if (this.focusedItemIndex === 1) cancelRow.addClass('is-focused');
    cancelRow.createSpan({ text: this.focusedItemIndex === 1 ? '\u203A' : '\u00A0', cls: 'codian-ask-cursor' });
    cancelRow.createSpan({ text: '2. ', cls: 'codian-ask-item-num' });
    cancelRow.createSpan({ text: 'Cancel', cls: 'codian-ask-item-label' });
  ...
  private renderMultiSelectCheckbox(parent: HTMLElement, checked: boolean): void {
    parent.createSpan({
      text: checked ? '[\u2713] ' : '[ ] ',
      cls: `codian-ask-check${checked ? ' is-checked' : ''}`,
    });
  }
  ...
  private updateOptionVisuals(qIdx: number): void {
    ...
    for (let i = 0; i < q.options.length; i++) {
      ...
      if (isMulti) {
        const checkSpan = item.querySelector('.codian-ask-check') as HTMLElement | null;
        if (checkSpan) {
          checkSpan.textContent = isSelected ? '[\u2713] ' : '[ ] ';
          checkSpan.toggleClass('is-checked', isSelected);
        }
      } else {
        const labelRow = item.querySelector('.codian-ask-label-row') as HTMLElement | null;
        const existingMark = item.querySelector('.codian-ask-check-mark');
  ...
  private updateTabIndicators(): void {
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tick = tab.querySelector('.codian-ask-tab-tick');
      const answered = this.isQuestionAnswered(idx);
    ...
    if (submitTab) {
      const submitCheck = submitTab.querySelector('.codian-ask-tab-submit-check');
      const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));
