 ...
 *
 * Extracts options-building logic from CodianService for:
...
import type { CodianSettings, PermissionMode } from '../types';
 ...
 * Passed to builder methods to avoid direct dependencies on CodianService.
 */
...
export class QueryOptionsBuilder {
  ...
  static needsRestart(
    ...
    // Note: Permission mode is handled dynamically via setPermissionMode() in CodianService.
  ...
  static buildColdStartQueryOptions(ctx: ColdStartQueryContext): Options {
    ...
    const options: Options = {
      ...
      abortController: ctx.abortController,
      pathToClaudeCodeExecutable: ctx.cliPath,
      // User settings may contain permission rules that bypass Codian's permission system
      settingSources: ctx.settings.loadUserClaudeSettings
        ? ['user', 'project']
        : ['project'],
      env: {
  ...
  private static applyExtraArgs(options: Options, settings: CodianSettings): void {
