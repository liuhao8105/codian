...
export class PluginManager {
  ...
  async loadPlugins(): Promise<void> {
    ...
    const plugins: CodianPluginInfo[] = [];
    ...
    if (installedPlugins?.plugins) {
      for (const [pluginId, entries] of Object.entries(installedPlugins.plugins)) {
        ...
        if (!Array.isArray(entries)) {
          new Notice(`Codian: plugin "${pluginId}" has malformed entry in installed_plugins.json (expected array, got ${typeof entries})`);
        }
