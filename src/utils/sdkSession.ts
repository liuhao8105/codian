...
async function readCodexSession(sessionId: string): Promise<CodexSessionReadResult> {
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : String(error);
    return { entries: [], skippedLines: 0, error: errorMsg };
  }
}

function parseCodexTimestamp(timestamp: string | undefined): number {
  if (!timestamp) return Date.now();
  const parsed = Date.parse(timestamp);
  return Number.isFinite(parsed) ? parsed : Date.now();
}

function extractCurrentNoteFromCodexPrompt(text: string): string | undefined {
  const match = text.match(/<current_note>\n([\s\S]*?)\n<\/current_note>/);
  return match?.[1]?.trim() || undefined;
}

async function loadCodexLocalImages(localImages: unknown, timestamp: number): Promise<ImageAttachment[] | undefined> {
  ...
  for (let i = 0; i < localImages.length; i++) {
    ...
    try {
      ...
      // Ignore unreadable local images - text history is still useful.
    }
  }
...
async function loadCodexSessionMessages(sessionId: string): Promise<SDKSessionLoadResult> {
  ...
  for (let index = 0; index < result.entries.length; index++) {
    ...
    if (payloadType === 'user_message') {
      const rawContent = typeof entry.payload.message === 'string' ? entry.payload.message.trim() : '';
      if (!rawContent) continue;
      chatMsg = {
        id: `codex-user-${sessionId}-${index}`,
        role: 'user',
        content: rawContent,
        displayContent: extractContentBeforeXmlContext(rawContent) ?? rawContent,
        timestamp,
        currentNote: extractCurrentNoteFromCodexPrompt(rawContent),
        images: await loadCodexLocalImages(entry.payload.local_images, timestamp),
      };
...
export async function loadSDKSessionMessages(
  ...
  const sdkSessionPath = (() => {
    try {
      return getSDKSessionPath(vaultPath, sessionId);
    } catch {
      return null;
    }
  })();

  if (!sdkSessionPath || !existsSync(sdkSessionPath)) {
    return await loadCodexSessionMessages(sessionId);
  }
