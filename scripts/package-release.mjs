...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === '--root' || argument === '--rollback-dir') {
...
export function packageRelease({ root, rollbackDir }) {
  const resolvedRoot = resolve(root);
  const resolvedRollback = resolve(rollbackDir);
  requireReleaseFiles(resolvedRoot, 'Release source');
  requireReleaseFiles(resolvedRollback, 'Rollback source');
  ...
  const rollbackVersion = readJson(join(resolvedRollback, 'manifest.json')).version;
  ...
  if (typeof rollbackVersion !== 'string' || rollbackVersion === packageVersion) {
    throw new Error('Rollback manifest must identify a different, valid version');
  }
  ...
  const rollbackArchive = join(
    outputDirectory,
    `codian-${packageVersion}-rollback.zip`,
  );
  buildArchive(resolvedRoot, installArchive);
  buildArchive(resolvedRollback, rollbackArchive);
  ...
  return { installArchive, rollbackArchive, checksumPath };
}

try {
  const args = parseArgs(process.argv.slice(2));
  const root = args.root ?? process.cwd();
  const rollbackDir = args['rollback-dir'] ?? process.env.CODIAN_ROLLBACK_DIR;
  if (!rollbackDir) {
    throw new Error('--rollback-dir or CODIAN_ROLLBACK_DIR is required');
  }
  const result = packageRelease({ root, rollbackDir });
  process.stdout.write(
    `Packaged ${basename(result.installArchive)} and ${basename(result.rollbackArchive)}\n`,
  );
} catch (error) {
